    ...
    for layer_name in CNN_structure:
        ...
        if layer_name == var_layers[layer_index].name[var_layers[layer_index].name.find("/") + 1:]:
            ...
            for content_seg, style_seg in zip(content_segs, style_segs):
                ...
                diff_style_sum    = tf.reduce_mean(tf.squared_difference(gram_matrix_const, gram_matrix_var)) \
                     * content_mask_mean
...
def stylize(args, Matting):
    ...
    # saver.save(sess, "C:/Study/deep-photo-styletransfer-tf-master/checkpoint/my_test_model", global_step=10) 
    start = time.time()
    ...
    style_layers_var = [vgg_var.conv1_1, vgg_var.conv2_1, vgg_var.conv3_1, vgg_var.conv4_1, \
                                                                            vgg_var.conv5_1]
    ...
    # Style Loss
    loss_styles_list = style_loss(layer_structure_all, style_layers_const, style_layers_var, \
                                        content_masks, style_masks, float(args.style_weight))
    loss_style = 0.0
    ...
    if args.lbfgs:
        if not Matting:
            overall_loss = loss_content + loss_style + loss_tv 
        else:
